...
import { CheckCircle2, Lock, Loader2, ChevronDown, Clock, ArrowDown } from 'lucide-react'
...
function TaskRow({ task }: { task: OnboardingTaskRow }) {
  ...
  return (
    <li className="flex items-center gap-2.5 rounded-lg border border-carbon-700 bg-carbon-800/40 px-3 py-2">
      ...
      <span className={`flex-1 text-sm ${done ? 'text-slate-500 line-through' : 'text-slate-200'}`}>{task.title}</span>
